...
export function OrderingQuestion({ options, order, onChange, disabled = false, correctOrder }: OrderingQuestionProps) {
  const defaultOrder = options.map((option) => option.label)
  const labels = order.length === options.length ? order : defaultOrder
  const optionByLabel = new Map(options.map((option) => [option.label, option]))
  const initializedFor = useRef<string | null>(null)

  useEffect(() => {
    if (disabled || order.length > 0 || options.length === 0) return
    const key = options.map((option) => option.label).join('|')
    if (initializedFor.current === key) return
    initializedFor.current = key
    onChange(defaultOrder)
  }, [defaultOrder, disabled, onChange, options, order.length])
